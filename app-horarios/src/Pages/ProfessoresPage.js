...
function ProfessoresPage() {
  ...
  useEffect(() => {
    localStorage.setItem("gradeBlocos", JSON.stringify([{ type: "professor", data: gradeBlocos }]));
  }, [gradeBlocos]);

  const exportExcel = () => {
    const ws = XLSX.utils.json_to_sheet(gradeBlocos);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "HorarioProfessor");
    XLSX.writeFile(wb, "horario_professor.xlsx");
  };
        ...
        <DragDropContext onDragEnd={handleDragEnd}>
          <div className="row">
            <div className="col-md-9">
              <h2 className="mb-4 pt-3">Horário Professor</h2>
